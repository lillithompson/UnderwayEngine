...
export function TintBar({ title = 'TINT', removeLabel, tint, solidOnly, onChange, onCommit, onBack, onRemove, onPickColor, onAddStop, onSheetOpenChange }: {
  ...
  // Solid-only: the bar reads AND writes the tint as a solid at Normal blend,
  // so the first committed edit flattens a legacy gradient or blended fill
  // (the model's retained `solid` color) rather than silently keeping an
  // uneditable ramp or blend alive.
